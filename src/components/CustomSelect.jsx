...
export default function CustomSelect({ value, onChange, options, placeholder = "Pilih...", className = "", icon: Icon, searchable = false, disabled = false, menuPosition = "absolute" }) {
  ...
  return (
    <div className={`relative ${className}`} ref={popoverRef}>
      ...
      {isOpen && (
        <div className={`${menuPosition === 'absolute' ? 'absolute right-0 left-0 z-[9999]' : 'relative w-full'} mt-2 p-2 min-w-[200px] bg-background/95 backdrop-blur-md rounded-xl shadow-2xl border border-card-border animate-in fade-in zoom-in-95 duration-200 max-h-60 flex flex-col`}>
          {searchable && (
