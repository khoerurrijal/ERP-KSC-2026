  ...
  return (
    <div className={`relative ${className}`} ref={popoverRef}>
      <button 
        ...
        className={`glass-input h-10 px-4 w-full flex items-center justify-between gap-2 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-primary/50 ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/10'}`}
      >
      ...
      {isOpen && (
        <div className="absolute right-0 left-0 mt-2 p-2 min-w-[200px] bg-[#18181B] rounded-xl shadow-2xl border border-white/10 z-[9999] animate-in fade-in zoom-in-95 duration-200 max-h-60 flex flex-col">
          {searchable && (
            <div className="p-2 border-b border-white/10 mb-2 shrink-0 relative">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-md h-8 pl-8 pr-3 text-xs text-foreground focus:outline-none focus:border-primary/50"
              />
            </div>
          )}
